instructions = """turn off 660,55 through 986,197
...
def parse_line(line):
	...
	if len(tokens) == 5:
		tokens = tokens[1:]
	return tokens[0], [int(x) for x in tokens[1].split(',')], [int(x) for x in tokens[3].split(',')]

def make_grid(rows=1000, columns=1000):
	return [[False for cell in range(columns)] for line in range(rows)]
...
def run_command(command, grid, light_function):
	action, first, second = parse_line(command)
	for y in range(first[1], second[1] + 1):
		for x in range(first[0], second[0] + 1):
			grid = light_function(grid, action, x, y)
	return grid

def run_instructions(instructions, grid, light_function):
	for instruction in instructions:
		grid = run_command(instruction, grid, light_function)
	return grid

# grid = make_grid()
# grid = run_instructions(instructions, grid, set_light)
# print "number of lights on:", count_lights(grid)

# PART 2

def make_number_grid(rows=1000, columns=1000):
	return [[0 for cell in range(columns)] for line in range(rows)]

def set_brightness(grid, action, y, x):
	if action == 'on':
		grid[x][y] += 1
	if action == 'off':
		if grid[x][y] > 0:
...
grid = make_number_grid()
grid = run_instructions(instructions, grid, set_brightness)
print "number of lights on:", count_brightness(grid)
