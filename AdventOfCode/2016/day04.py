...
class room():
	def __init__(self, room):
		self.full_room = room
		self.checksum = room[-6:-1]
		self.letters = {}
		self.sector_id = int(room[:-7].split('-')[-1])
		self.decrypted = ""

	def parse_letters(self):
		if len(self.letters) == 0:
			for char in self.full_room[:-7]:
				if char in string.ascii_lowercase:
					self.letters[char] = self.letters.get(char, 0)
					self.letters[char] += 1
	
	def most_used_letters(self):
		self.parse_letters()
		sorted_keys = sorted(self.letters.items(), key=lambda item: (-item[1], item[0]))
		letters = "".join([item[0] for item in sorted_keys[:5]])
		return letters

	def isValidRoom(self):
		return self.checksum == self.most_used_letters()

	def decrypt_string(self, encrypted):
		decrypted = ''
		for char in encrypted:
			loc = string.ascii_lowercase.index(char)
			new_loc = (loc + self.sector_id) % 26
			decrypted += string.ascii_lowercase[new_loc]
		return decrypted

	def decrypt_room(self):
		if self.decrypted == "":
			self.decrypted = ' '.join([self.decrypt_string(part) for part in self.full_room[:-7].split('-')[:-1]])
		return self.decrypted


rooms = rooms.split("\n")

assert room('aaaaa-bbb-z-y-x-123[abxyz]').isValidRoom()
assert room('a-b-c-d-e-f-g-h-987[abcde]').isValidRoom()
assert room('not-a-real-room-404[oarel]').isValidRoom()
assert not room('totally-real-room-200[decoy]').isValidRoom()

sector_sum = 0
for line in rooms:
	r = room(line)
	if r.isValidRoom():
		sector_sum += r.sector_id
		if r.decrypt_room().startswith('north'):
			print r.sector_id, r.decrypted
